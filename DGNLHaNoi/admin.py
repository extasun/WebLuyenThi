...
class CauHoiAdmin(admin.ModelAdmin):
    list_display = ('noi_dung', 'loai_cau_hoi', 'phan1', 'subject', 'phan2', 'ch_photo')
    list_filter = ('loai_cau_hoi', 'phan1', 'subject', 'phan2')
    inlines = [DapAnInline]
...
class NoiDungDeInline(admin.TabularInline):
    model = NoiDungDe
    extra = 1
class Phan1Inline(admin.TabularInline):
    model = NoiDungDe
    extra = 1
    ...
    def get_queryset(self, request):
        ...
        return qs.filter(cau_hoi__phan1='p1')
class Phan2Inline(admin.TabularInline):
    model = NoiDungDe
    extra = 1
    fields = ('cau_hoi', 'diem_so','thu_tu_cau')
    readonly_fields = ('id',)
    ...
    def get_queryset(self, request):
        qs = super().get_queryset(request)
        return qs.filter(cau_hoi__phan2='p2')
...
class DeThiAdmin(admin.ModelAdmin):
    ...
    inlines = [Phan1Inline, Phan2Inline, Phan3Inline]
